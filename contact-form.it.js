...
export function initContactForm() {
  ...
  /* Cartiglio: Datum live */
  const date = document.getElementById("cf-date");
  if (date) date.textContent = new Date().toLocaleDateString("it-IT", { day: "2-digit", month: "2-digit", year: "numeric" });
  ...
  function restoreSendBtn() {
    setTimeout(() => { /* erst nach dem Overlay-Fade zurücksetzen */
      if (labelEl) labelEl.textContent = "Invia il messaggio";
  ...
  form.addEventListener("submit", ev => {
    ...
    const finish = ok => {
      ...
      if (ok) { showDone(); restoreSendBtn(); }
        ...
        showError("Impossibile inviare il messaggio. Riprovare più tardi o scrivere direttamente a sales@machineering.com.");
      }
    };
    ...
    if (labelEl) labelEl.textContent = "Invio in corso \u2026";
