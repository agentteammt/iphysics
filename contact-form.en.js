...
export function initContactForm() {
  ...
  /* Title block: Datum live */
  const date = document.getElementById("cf-date");
  if (date) date.textContent = new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric" });
  ...
  form.addEventListener("submit", ev => {
    ...
    const finish = ok => {
      ...
      if (ok) { showDone(); restoreSendBtn(); }
        ...
        showError("The message could not be sent. Please try again later or write directly to sales@machineering.com.");
      }
    };
