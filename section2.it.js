...
export function initSection2() {
  ...
  /* ---------- Count-ups ---------- */
  const fmt = (n, sep) => sep ? Math.round(n).toLocaleString("it-IT") : String(Math.round(n));
  ...
  function layout() {
    ...
    /* Prenotazione demo (Änd. 3, 06.07.): Desktop zweispaltig, mobil untereinander — Höhe des
...
function showConsent(box) {
  ...
  card.innerHTML =
    '<div style="font-family:\'Titillium Web\',sans-serif;font-weight:600;font-size:11px;letter-spacing:0.26em;color:#6B7E86;text-transform:uppercase;">Video esterno</div>' +
    '<p style="margin:0;max-width:520px;font-family:\'Titillium Web\',sans-serif;font-weight:400;font-size:14px;line-height:1.65;color:#6B7E86;">Il caricamento del video comporta la trasmissione di dati al fornitore video. Il contenuto viene caricato solo con il Suo consenso.</p>' +
    '<button type="button" data-load style="font-family:\'Titillium Web\',sans-serif;font-weight:700;font-size:14px;letter-spacing:0.01em;color:#fff;background:linear-gradient(120deg,#3BAED1,#45B347);border:0;padding:13px 26px;border-radius:999px;cursor:pointer;">Carica e riproduci il video</button>';
...
function hint(box) {
  ...
  t.textContent = "Fonte video in arrivo — asset B";
