/* ============================================================================
   booking-widget.js — iPhysics Prenotazione demo (Custom Element <iph-booking>)
   ----------------------------------------------------------------------------
...
(function () {
  ...
  class IphBooking extends HTMLElement {
    ...
    async loadAvail() {
      ...
      } catch (e) { this.state.error = "Impossibile caricare le disponibilità. Ricaricare la pagina."; }
      this.render();
    }
    ...
    async submit() {
      ...
      try {
        if (live()) {
          ...
          if (!row || row.slot_id == null) { s.error = "Questo appuntamento non è più disponibile."; s.selTime = null; }
          else {
            ...
            if (r === "ok") s.done = true;
            else if (r === "full") { s.error = "Questo orario è appena stato occupato. Sceglierne un altro."; s.selTime = null; await this.loadAvail(); }
            else s.error = "Questo appuntamento non è più disponibile.";
          }
        } else {
    ...
    timesHTML() {
      ...
      if (!times.length) return `<div class="empty">Nessun orario disponibile in questo giorno.</div>`;
      if (live() && this.state.avail === null) return `<div class="empty">Caricamento disponibilità …</div>`;
      return times.map(t => {
    ...
    fieldsHTML() {
      ...
      return `<div class="fields">
        <label class="f"><span>Name *</span><input data-k="name" type="text" autocomplete="name" placeholder="Nome e cognome" value="${esc(f.name)}"></label>
        <label class="f"><span>Azienda</span><input data-k="company" type="text" autocomplete="organization" placeholder="La vostra azienda" value="${esc(f.company)}"></label>
        <label class="f full"><span>E-mail *</span><input data-k="email" type="email" autocomplete="email" placeholder="nome@azienda.it" value="${esc(f.email)}"></label>
        <label class="f full"><span>Messaggio (facoltativo)</span><textarea data-k="note" placeholder="Il vostro messaggio">${esc(f.note)}</textarea></label>
      ...
      <label class="privacy"><input data-k="privacy" type="checkbox" ${this.state.privacy ? "checked" : ""}>
        <span>Acconsento al trattamento dei miei dati da parte di machineering per fissare l’appuntamento. <a href="Datenschutz.dc.html">informativa sulla privacy</a></span></label>`;
    }
    ctaHTML() {
      const ok = this.valid() && !this.state.submitting;
      return `<div class="row">
        <button class="cta" data-go type="button" ${ok ? "" : "disabled"}>${this.state.submitting ? "Invio richiesta …" : "Prenota l’appuntamento"} <span aria-hidden="true">→</span></button>
        <div class="foot">${live() ? "Disponibilità in tempo reale · conferma via e-mail" : "Modalità demo — nessun dato salvato"}</div>
    ...
    doneHTML() {
      return `<div class="done"><div class="mark"><div>${CHECK}</div></div>
        ...
        <p><span class="when">${esc(this.selLabel())} · ${esc(this.state.selTime || "")}</span> — riceverete la conferma via e-mail.</p>
        <button class="ghost" data-again type="button">Scegli un altro appuntamento</button></div>`;
    }
    corners() {
      ...
      if (s.done) body = this.doneHTML();
        ...
        body = `<div class="cols">
          <div class="left">
            ...
            <p class="step" style="margin-top:14px">Scegli il giorno</p>
            <div class="days">${this.daysHTML()}</div>
            <div class="summary"><div class="lbl">Il vostro appuntamento</div>
              <div class="val">${s.selTime ? esc(this.selLabel()) + " · " + esc(s.selTime) : esc(this.selLabel())}</div></div>
          </div>
          <div class="right">
            <div class="head"><b>Primo colloquio · ${DURATION}</b><span class="meta">gratuito · da remoto</span></div>
            <div class="sec"><p class="step">Scegli l’orario</p><div class="times">${this.timesHTML()}</div></div>
            <div class="sec"><p class="step">Contatto</p>${this.fieldsHTML()}</div>
            ${this.ctaHTML()}
          </div></div>`;
      } else if (this.variant === "v3") {
        body = `<div class="pad">
          <div class="head"><b>Prenota l’appuntamento · ${DURATION}</b><span class="meta">Primo colloquio · remote</span></div>
          <div class="sec"><p class="step">Giorno</p><div class="days">${this.daysHTML()}</div></div>
          <div class="sec"><p class="step">Orario</p><div class="times">${this.timesHTML()}</div></div>
          <div class="sec"><p class="step">Contatto</p>${this.fieldsHTML()}</div>
          ${this.ctaHTML()}</div>`;
      } else if (this.variant === "v4") {
        body = `<div class="pad">${this.corners()}
          <div class="head"><div><div class="kicker">Prenotazione demo</div><b style="display:block;margin-top:8px">Primo colloquio · ${DURATION}</b></div><span class="meta">gratuito · da remoto</span></div>
          <div class="sec"><p class="step">01 · Scegli il giorno</p><div class="days">${this.daysHTML()}</div></div>
          <div class="sec"><p class="step">02 · Scegli l’orario</p><div class="times">${this.timesHTML()}</div></div>
          <div class="divider"></div>
          <div class="sec"><p class="step">03 · Contatto</p>${this.fieldsHTML()}</div>
          ${this.ctaHTML()}</div>`;
      } else { /* v1 */
        body = `<div class="pad">
          <div class="head"><div><div class="kicker">Prenotazione demo</div><b style="display:block;margin-top:8px">Primo colloquio · ${DURATION}</b></div><span class="meta">gratuito · da remoto</span></div>
          <div class="sec"><p class="step">1 · Scegli il giorno</p><div class="days">${this.daysHTML()}</div></div>
          <div class="sec"><p class="step">2 · Scegli l’orario</p><div class="times">${this.timesHTML()}</div></div>
          <div class="sec"><p class="step">3 · Contatto</p>${this.fieldsHTML()}</div>
          ${this.ctaHTML()}</div>`;
      }
