/* ============================================================================
   booking-widget.js — iPhysics Demo booking (Custom Element <iph-booking>)
   ----------------------------------------------------------------------------
...
(function () {
  ...
  class IphBooking extends HTMLElement {
    ...
    async loadAvail() {
      ...
      } catch (e) { this.state.error = "Availability could not be loaded. Please reload the page."; }
      this.render();
    }
    ...
    async submit() {
      ...
      try {
        if (live()) {
          ...
          if (!row || row.slot_id == null) { s.error = "This slot is no longer available."; s.selTime = null; }
          else {
            ...
            if (r === "ok") s.done = true;
            else if (r === "full") { s.error = "This slot has just been taken. Please pick another."; s.selTime = null; await this.loadAvail(); }
            else s.error = "This slot is no longer available.";
          }
        } else {
      ...
      } catch (e) { s.error = "Booking failed. Please try again."; }
    ...
    timesHTML() {
      ...
      if (!times.length) return `<div class="empty">No slots on this day.</div>`;
      if (live() && this.state.avail === null) return `<div class="empty">Loading availability …</div>`;
      return times.map(t => {
    ...
    fieldsHTML() {
      ...
      return `<div class="fields">
        <label class="f"><span>Name *</span><input data-k="name" type="text" autocomplete="name" placeholder="First and last name" value="${esc(f.name)}"></label>
        <label class="f"><span>Company</span><input data-k="company" type="text" autocomplete="organization" placeholder="Your company" value="${esc(f.company)}"></label>
        <label class="f full"><span>Email *</span><input data-k="email" type="email" autocomplete="email" placeholder="name@company.com" value="${esc(f.email)}"></label>
        <label class="f full"><span>Message (optional)</span><textarea data-k="note" placeholder="Your message">${esc(f.note)}</textarea></label>
      ...
      <label class="privacy"><input data-k="privacy" type="checkbox" ${this.state.privacy ? "checked" : ""}>
        <span>I agree that machineering may process my details to arrange this appointment. <a href="Datenschutz.dc.html">privacy policy</a></span></label>`;
    }
    ctaHTML() {
      const ok = this.valid() && !this.state.submitting;
      return `<div class="row">
        <button class="cta" data-go type="button" ${ok ? "" : "disabled"}>${this.state.submitting ? "Requesting …" : "Book slot"} <span aria-hidden="true">→</span></button>
        <div class="foot">${live() ? "Real-time availability · confirmation by email" : "Demo mode — nothing is stored"}</div>
      </div>${this.state.error ? `<div class="err">${esc(this.state.error)}</div>` : ""}`;
    }
    doneHTML() {
      return `<div class="done"><div class="mark"><div>${CHECK}</div></div>
        <h3>Slot booked</h3>
        <p><span class="when">${esc(this.selLabel())} · ${esc(this.state.selTime || "")}</span> — you will receive a confirmation by email.</p>
        <button class="ghost" data-again type="button">Pick another slot</button></div>`;
    }
    corners() {
      ...
      if (s.done) body = this.doneHTML();
        ...
        body = `<div class="cols">
          <div class="left">
            ...
            <p class="step" style="margin-top:14px">Pick a day</p>
            <div class="days">${this.daysHTML()}</div>
            <div class="summary"><div class="lbl">Your slot</div>
              <div class="val">${s.selTime ? esc(this.selLabel()) + " · " + esc(s.selTime) : esc(this.selLabel())}</div></div>
          </div>
          <div class="right">
            <div class="head"><b>Intro call · ${DURATION}</b><span class="meta">free · remote</span></div>
            <div class="sec"><p class="step">Pick a time</p><div class="times">${this.timesHTML()}</div></div>
            <div class="sec"><p class="step">Contact</p>${this.fieldsHTML()}</div>
            ${this.ctaHTML()}
          </div></div>`;
      } else if (this.variant === "v3") {
        body = `<div class="pad">
          <div class="head"><b>Book slot · ${DURATION}</b><span class="meta">Intro call · remote</span></div>
          <div class="sec"><p class="step">Day</p><div class="days">${this.daysHTML()}</div></div>
          <div class="sec"><p class="step">Time</p><div class="times">${this.timesHTML()}</div></div>
          <div class="sec"><p class="step">Contact</p>${this.fieldsHTML()}</div>
          ${this.ctaHTML()}</div>`;
      } else if (this.variant === "v4") {
        body = `<div class="pad">${this.corners()}
          <div class="head"><div><div class="kicker">Demo booking</div><b style="display:block;margin-top:8px">Intro call · ${DURATION}</b></div><span class="meta">free · remote</span></div>
          <div class="sec"><p class="step">01 · Pick a day</p><div class="days">${this.daysHTML()}</div></div>
          <div class="sec"><p class="step">02 · Pick a time</p><div class="times">${this.timesHTML()}</div></div>
          <div class="divider"></div>
          <div class="sec"><p class="step">03 · Contact</p>${this.fieldsHTML()}</div>
          ${this.ctaHTML()}</div>`;
      } else { /* v1 */
        body = `<div class="pad">
          <div class="head"><div><div class="kicker">Demo booking</div><b style="display:block;margin-top:8px">Intro call · ${DURATION}</b></div><span class="meta">free · remote</span></div>
          <div class="sec"><p class="step">1 · Pick a day</p><div class="days">${this.daysHTML()}</div></div>
          <div class="sec"><p class="step">2 · Pick a time</p><div class="times">${this.timesHTML()}</div></div>
          <div class="sec"><p class="step">3 · Contact</p>${this.fieldsHTML()}</div>
          ${this.ctaHTML()}</div>`;
      }
