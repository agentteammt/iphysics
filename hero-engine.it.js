...
const GLB_FALLBACK_BYTES = { desktop: 5.3e6, mobile: 3.38e6 }; /* B6 (09.07.): v2-Dateigrößen für den Avanzamentos-Fallback */
...
export async function initHero(cfg = {}) {
  ...
  hint.innerHTML = isTouch
    ? '<b style="color:#3BAED1;font-weight:700">SCORRETE</b> — LA LENTE MOSTRA L’IMPIANTO REALE'
    : 'LA <b style="color:#3BAED1;font-weight:700">LENTE</b> SEGUE IL MOUSE — MOSTRA L’IMPIANTO REALE';

  /* Chrome-Layout (v6): Cartiglio-Höhe, Mobil-Varianten, Logo-Größe, Readout unter dem Logo */
  ...
  /* EINE Kamera für alle Layer (Wireframe, Real, Bake) → Lens-Registrierung
     bleibt pixelgenau. mult 1 = Hero-Framing, 0 = zentriert; die Tour del processo
  ...
  /* ---------- GLB laden (echter LoadingManager-Avanzamento) ---------- */
  let loadDone = false, loadError = null;

  /* Änd. 3 (06.07.): „SISTEMA PRONTO"-Puls — einmaliger Glanz-Sweep (~400 ms) durch die
  ...
  function setProgress(p) {
    ...
    if (p >= 1) {
      progLabel.textContent = "SISTEMA PRONTO"; /* Label-Wechsel auch bei reduced-motion; Fehler-Label unberührt */
  ...
  async function load3D() {
    ...
    camReadHero = "REFERENZA: MACCHINA DI CONWEX GmbH";
    ...
    /* Shader-Warm-up (07.07.): beide GL-Kontexte VOR dem ersten sichtbaren Frame kompilieren
       ...
       erste Bake auf schwachen iGPUs genau den „SISTEMA PRONTO"-Moment ein. */
    setProgress(.97);
    try {
  ...
  /* Element 2 — Passermarken: Rahmen-Ecken der Zeichnungsfläche zwischen
     Header (78 px) und Cartiglio (barH) — berühren Logo/Skip/Leiste nicht. */
  ...
  const fmtMM = m => (Math.round(m * 100) * 10).toLocaleString("it-IT"); /* 4,83 m → „4.830" */
  ...
  function projectAnnotations() {
    ...
    dim(V(mx.x, mn.y, mn.z), V(mx.x, mx.y, mn.z), V(1, 0, 0), .3, [DIM_LOCK.y, "ASSE Z"], 1.15); /* Höhe, hintere rechte Kante */

    /* Callout „CELLA 01" mit Fahnenlinie — nach oben links, weg vom Tiefen-Maß */
    (() => {
      const p0 = pr(V(cc.x - size.x * .10, mx.y - size.y * .05, cc.z + size.z * .30));
      const e = { x: p0.x - 30, y: p0.y - 26 }, e2 = { x: e.x - 16, y: e.y };
      if (!inZone(p0, 10) || !inZone({ x: e2.x - 84, y: e2.y }, 10)) { console.log("[ann] Callout CELLA 01 unterdrückt"); return; }
      const g = svgEl("g", {});
      annFade(g, 1.3);
      annSvg.appendChild(g);
      ...
      g.append(l1, l2);
      annFade(annLabel(e2.x - 7, e2.y, ["CELLA 01"], "r"), 1.62);
    })();
    ...
    const NODES = [
      { f: [-.16, .80, -.12], lbl: "ASSE 3" }, /* Robotergelenk */
  ...
  /* ---------- Intro v4.1: KPI-Sequenz (§3) — „Blueprint-Stack" (Änd. 1+2, 06.07.) ----------
     ...
     Dock-Flug, Mini-Dissolve im Cartiglio und alle Sequenz-Timings bleiben unverändert. */
  const KPIS = [["+40%", "TASSO DI CHIUSURA VENDITE"], ["−50%", "COSTI DI ENGINEERING"],
                ["−75%", "TEMPO DI COMMISSIONING"]];
  ...
  let beatStarted = false, introSkipped = false, skipAll = false, readyShownAt = 0, readyDelay = 0; /* Änd. 1 (06.07.): Abflug-Beat mit großem „SISTEMA PRONTO" */
  const READY_BEAT = 60; /* 07.07.: „SISTEMA PRONTO" folgt DIREKT auf das Verschwinden der letzten Zahl (Ankunft im Cartiglio) */
  ...
  /* Abflug-Herzschlag — alle drei Slot-Werte pulsieren einmal SYNCHRON (scale ~1.08,
     ...
     „SISTEMA PRONTO" und läuft in dessen Build/Hold hinein. */
  ...
  function pendingLaunch() { /* 09.07.: Wartefall — GLB/Video beim Finale-Trigger noch nicht bereit.
       ...
       ist, öffnet die Iris direkt und die Video-Zoomfahrt startet. Kein „Sistema pronto"-Wort mehr. */
  ...
  function launch() {
    ...
    const IDUR = reduced ? 120 : 620; /* Auto-Iris 620 ms ab Bildschirmmitte — Cartiglio (z11) überlebt */
    let irSkip = true;
    (function ir() {
  ...
  /* Änd. 1 (06.07., v3.3): großes „SISTEMA PRONTO" als Abflug-Beat — mittig auf dem freien
  ...
  function showReadyWord(mode) {
    ...
    console.log(`[hero] Abflug-Beat „SISTEMA PRONTO" (${mode}) @ ${seqStart ? Math.round(performance.now() - seqStart) : 0} ms`);
  ...
  function showBeat() { /* 09.07.: nur noch der Abflug-Herzschlag — das große „Sistema pronto"-Wort entfällt */
  ...
  function tryLaunch() {
    ...
    pendingStart = true;
    showBeat();
    if (!loadDone) progLabel.textContent = "CARICAMENTO DIGITAL TWIN";
  }
  ...
  if (devSkip) { pendingStart = false; console.warn("[hero] devSkip aktiv — Intro & KPI-Zahlen übersprungen (Prop devSkipIntro oder QA-Flag 'skip' in sessionStorage iph_qa_flags)"); }
    ...
    /* Reduced Motion: Cartiglio sofort voll befüllt, kein Flug */
  ...
  /* ================= Abschnitt 1: Tour del processo (§7) =================
  ...
  function renderTour() {
    ...
    camRead.textContent = "REFERENZA: MACCHINA DI CONWEX GmbH";
  ...
  load3D().then(() => {
    ...
    progLabel.textContent = "ERRORE NEL CARICAMENTO DEL DIGITAL TWIN";
  });
